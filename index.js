...
function getCarInfoByIndex(inventory, index) {
  /* code here */
  const getCarInfo = inventory.find((item, index) => {
    return index === index;
  })
  return `This is a ${getCarInfo.car_make} ${getCarInfo.car_model}`
}
...
function sortCarInventory(inventory) {
  /* code here */
  const sortCar  = inventory.sort((a, b) => {
    const nameA = a.car_model.toUpperCase();  // ignore upper and lowercase
    const nameB = b.car_model.toUpperCase();  // ignore upper and lowercase
    if (nameA < nameB) {
      return -1;
    }
    if (nameA > nameB) {
...
function getModelYears(inventory) {
  /* code here */
  const modelYears = inventory.map((item) => {
...
function getOlderCars(inventory, maxYear) {
  /* code here */
  const olderCars = inventory.filter((item) => {
...
function getGermanCars(inventory) {
  /* code here */
  const germanCars = inventory.filter((item) => {
    return item.car_make == 'Audi' || item.car_make == 'Mercedes-Benz' || item.car_make == 'Volkswagen' || item.car_make == 'BMW';
  })
  return germanCars;
}
